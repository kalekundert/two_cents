...
def assign_debits(session, debits):
    if not debits:
        return
    elif len(debits) == 1:
    ...
    for debit in debits:
        ...
        while True:
            ...
            if explicit_value > debit.value:
                print("Too much money assigned.")
                continue

            if implicit_accounts:
                implicit_value = debit.value - explicit_value
            ...
            if sum(all_accounts.values()) < debit.value:
        ...
        print()
        #print(all_accounts)
        #debit.assign(session, all_accounts)
